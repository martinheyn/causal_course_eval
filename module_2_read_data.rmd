...
d_in$InEquality_bin <- as.integer(ifelse(d_in$Equality < 4.0, 2, 1))
...
save(d_in, d_in_s, file = "./data/data_r_in_num.RData")
```

# Make a simple correlation plot
```{r}
d_corr <- subset(d_in_s, select = -c(Swedish, CS, Course, year, study_period))

d_corr_matrix <- matrix(unlist(d_corr))
