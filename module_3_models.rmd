...
```{r}
dlist <- list(
      Population = as.integer(d_in$Survey_population),
      Returned = as.integer(d_in$Survey_returned),
...
model_m1 <- ulam(log_lik=TRUE,
      ...
      mu <- a_s[StudyPeriod] + a[Swedish] + b_p * Prereq_s + b_l * LearningOut_s + b_s * Structure_s + b_t * Teaching_s + b_li * Literature_s + b_a * Assessment_s + b_ad * Admin_s + b_w * Workload_s + b_e * Equality_s,
      a_s[StudyPeriod] ~ dnorm(0, 1),
      a[Swedish] ~ dnorm(0, 1),
      b_p ~ dnorm(0, 1),
      b_l ~ dnorm(0, 1),
      b_s ~ dnorm(0, 1),
...
```{r}
model_mtreturn <- ulam(log_lik=TRUE,
      alist(
      Returned ~ dbinom(Population, p),
      logit(p) <- a[StudyPeriod] + b_o * Overall_s + b_p * Prereq_s + b_l * LearningOut_s + b_s * Structure_s + b_t * Teaching_s + b_li * Literature_s + b_a * Assessment_s + b_ad * Admin_s + b_w * Workload_s + b_e * Equality_s,
